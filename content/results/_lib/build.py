...
RESULTS = HERE.parent
# Build artifacts served verbatim by the Next.js app (PDF, TeX, prompt).
PUBLIC = HERE.parent.parent.parent / "public" / "results"
...
def copy_artifacts(d: Path, slug: str, meta: dict) -> None:
    """Copy every locally-linked artifact (PDF, TeX, prompt, verifier) into
    public/results/<slug>/ so the deep links the header renders keep working."""
    for _, url in meta["links"]:
        if url.startswith(("http://", "https://", "/")):
            continue
        src = d / url
        if not src.exists():
            raise BuildError(f"meta.json links missing local file {url!r}")
        dst = PUBLIC / slug / url
...
def build(slug: str) -> dict:
    ...
    # A paper with its own build.py (erdos-690, which predates this pipeline)
    # builds itself; it emits the same article.html fragment shape.
    own_build = d / "build.py"
    if own_build.exists():
        proc = subprocess.run(
            [sys.executable, str(own_build)], capture_output=True, text=True
        )
        if proc.returncode != 0:
            raise BuildError(f"own build.py failed: {proc.stderr.strip()[:800]}")
        copy_artifacts(d, slug, meta)
        return {
            "slug": slug,
            "theorems": [],
            "equations": [],
    ...
    # Fragment consumed by app/results/[slug]/page.tsx at build time; the
    # header (title, badge, links) renders from meta.json as React there.
    fragment = (
